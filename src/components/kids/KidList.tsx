  ...
  if (kids.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-700 mb-4">No kids added yet.</p>
        <Link
          href="/kids/new"
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {kids.map((kid) => (
        ...
        >
          ...
          {kid.grade && (
            <p className="text-gray-700 text-sm mb-1">Grade: {kid.grade}</p>
          )}
          {kid.birthday && (
          ...
          {kid.notes && (
            <p className="text-gray-600 text-sm mt-2 line-clamp-2">{kid.notes}</p>
          )}
        </Link>
      ))}
    </div>
